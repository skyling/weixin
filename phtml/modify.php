	 ...
	 if (isset($_POST['table'])) {
		switch ($_POST['table']) {
			case 'lib_cmd':
				...
				break;
			case 'lib_content':
				$serverName='http://hnist.sinaapp.com/images/'; 
		...
		<?php
			...
			if ($ans_arr) {
				foreach ($ans_arr as $key => $value) {
					...
					echo "标题:<input type='text' name='title' value='".$value[3]."' style='width:600px;' /><br /><br />";
					if ($typeFlag=='text') {
						echo "内容:<textarea cols='80' rows='3' name='con_content'>".$value[2]."</textarea><br /><br />";
					}
					echo "描述:<input type='text' name='description' value='".$value[4]."' style='width:600px;' /><br /><br />";
					echo "页面URL:<input type='text' name='url' value='".$value[6]."' style='width:600px;' /><br /><br />";
					echo "图片URL:<input type='text' name='psrc' value='".$value[5]."' style='width:600px;' readonly='true' /><br /><br />";
